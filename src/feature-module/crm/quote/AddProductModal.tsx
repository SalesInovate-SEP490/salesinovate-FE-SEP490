...
import { addListProductQuote, addPriceBookToQuote, getListProductAddToQuote, getListProductPriceBook, getListQuoteProduct } from "../../../services/quote";
import { searchProductToAdd } from "../../../services/opportunities";

const AddProductModal: React.FC<{ pbId?: any, getList?: any; quoteId?: any, opportunityId?: any }> = ({ pbId, getList, quoteId, opportunityId }) => {
        ...
        searchProductToAdd(param)
            .then(response => {
                if (response.code === 1) {
                    ...
                    setItems({
                        initItems: response.data.map((item: any) => { return { label: item?.product?.productName, value: item?.product?.productId } }),
                        items: response.data.map((item: any) => { return { label: item?.product?.productName, value: item?.product?.productId } })
                    });
                }
            })
            .catch(error => {
    ...
    const handleSave = () => {
        ...
        const listProducts = editableProduct.map(product => {
            return {
                ...
                sales_price: product?.unitPrice,
                line_description: product?.lineDescription,
            }
        });
        addListProductQuote(quoteId, listProducts)
            .then(response => {
                if (response.code === 1) {
                    if (getList)
                        getList();
    ...
    const handleChoosePriceBook = () => {
        addPriceBookToQuote(priceBookId, quoteId)
            .then(response => {
                if (response.code === 1) {
                    setStep(1);
    ...
    return (
        <div className="modal custom-modal fade" id="add_products_quote" role="dialog">
