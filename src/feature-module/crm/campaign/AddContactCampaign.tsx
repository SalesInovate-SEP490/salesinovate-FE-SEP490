...
import { addContactToCampaign, addLeadToCampaign, getListCampainMemberStatus } from "../../../services/campaign_member";
import Swal from "sweetalert2";
import { getListContact } from "../../../services/Contact";
import { toast } from "react-toastify";

const AddContactCampaign: React.FC<{
        ...
        getListContact(param)
            .then(response => {
                if (response.code === 1) {
                    ...
                    setItems({
                        initItems: leads?.map((item: any) => { return { label: item?.firstName + " " + item?.lastName, value: item.contactId } }),
                        items: leads?.map((item: any) => { return { label: item?.firstName + " " + item?.lastName, value: item.contactId } })
                    });
                }
            })
            .catch(error => {
    ...
    const columns = [
        {
            ...
            render: (value: undefined, record: Partial<any>) => {
                return <Link to={"/leads-details/" + record?.contactId}>{record?.firstName + " " + record?.lastName}</Link>
            }
        },
        {
    ...
    const handleSave = () => {
        Swal.showLoading();
        const listProducts = editableProduct.map(contact => {
            return {
        ...
        addContactToCampaign(listProducts)
            .then(response => {
                Swal.close();
                if (response.code === 1) {
                    toast.success("Add contact to campaign successfully!");
                    if (getList)
                        getList();
                    searchContactToAdd();
                    setStep(1);
                    setSelectedProductKeys([]);
                    document.getElementById("close-btn-add-contact-campaign")?.click();
                }
            })
            .catch(error => {
                Swal.close();
    ...
    return (
        <div className="modal custom-modal fade" id="add_contacts_campaign" role="dialog">
                    ...
                    <div className="modal-body text-center">
                        {
                            step === 1 &&
                            <>
                                <h4 className="modal-title">{t("CAMPAIGN.ADD_CONTACT_TO_CAMPAIGN")}</h4>
                            ...
                            <>
                                <div className="col-md-12">
                                    ...
                                    <div className="space-between">
                                        ...
                                        <div>
                                            <Link id="close-btn-add-contact-campaign" to="#" className="btn btn-light mr-1" data-bs-dismiss="modal">
                                                {t("ACTION.CANCEL")}
                                            </Link>
